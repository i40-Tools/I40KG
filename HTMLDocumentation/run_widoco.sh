...
java -jar widoco-1.4.12-jar-with-dependencies.jar -ontFile ontology.ttl -uniteSections -confFile config.properties -outFolder docs -rewriteAll
mv docs/index-en.html docs/index.html

# clean up namespaces:
ns_tab_start=$(grep -n -m 1 "<tbody>" docs/index.html | cut -d: -f1)
ns_tab_end=$(grep -n -m 1 "</tbody>" docs/index.html | cut -d: -f1)
...
{ head -n $(($ns_tab_start)) docs/index.html; cat prefix_ns_tab.html ; tail -n +$ns_tab_end docs/index.html; } > tmp_index.html
mv tmp_index.html docs/index.html

# replace standard introduction and description with something meaningful..
sed -i -e 's/This is a place holder text for the introduction\. The introduction should briefly describe the ontology, its motivation, state of the art and goals\./This documentation presents STO, an ontology to describe standards, organizations for publishing standards as well as standardization frameworks in the context of Industry 4\.0\./' docs/index.html
...
mkdir -p docs/serializations
mv docs/ontology.xml docs/serializations/
mv docs/ontology.ttl docs/serializations/
mv docs/ontology.json docs/serializations/
mv docs/ontology.nt docs/serializations/
